...
def train():
    ...
    logging.info(" ** learning rate: %f" % config.train.lr_init)
    ...
    for epoch in range(config.train.n_epoch):
        ...
        logging.info("[*] Epoch: [%2d/%2d] time: %4.4fs, loss: %.8f" % (epoch, config.train.n_epoch, time.time() - epoch_time, total_g_loss/n_iter))
...
def test(file):
    try:
        ...
        logging.error('cannot open %s'%(file))
    else:
        ...
        logging.info('Input size: %s,%s,%s'%(size[0],size[1],size[2]))
        ...
        logging.info("took: %4.4fs" % (time.time() - start_time))
